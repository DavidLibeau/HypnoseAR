...
public static class RuntimePreviewGenerator
{
    ...
    public static Texture2D GenerateMaterialPreviewWithShader(Material material, PrimitiveType previewPrimitive, Shader shader, string replacementTag, int width = 64, int height = 64)
    {
        ...
        try
        {
            return GenerateModelPreviewWithShader(previewModel.transform, Quaternion.identity, shader, replacementTag, width, height, false);
        }
        catch (Exception e)
        {
    ...
    public static Texture2D GenerateModelPreview(Transform model, Quaternion customRotation, int width = 64, int height = 64, bool shouldCloneModel = false)
    {
        return GenerateModelPreviewWithShader(model, customRotation, null, null, width, height, shouldCloneModel);
    }

    public static Texture2D GenerateModelPreviewWithShader(Transform model, Quaternion customRotation, Shader shader, string replacementTag, int width = 64, int height = 64, bool shouldCloneModel = false)
    {
        ...
        try
        {
            ...
            if (!isStatic)
            {
                ...
                previewObject.rotation = customRotation;
            }
